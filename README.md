...
- 左侧内置冒泡排序、链表、二叉树、邻接表、邻接矩阵 BFS 和动态堆数组示例
- 默认简洁的教学程序地图，完整控制流保留为可下钻视图
- Trace、Monaco、流程图、数组比较卡片和内存视图统一联动
- 常驻内存抽屉、栈/全局区/堆独立地址尺与当前步骤读写标记
- 可下钻的数组、矩阵、记录、链表、树、图和通用指针结构视图
- 变量观察器、逻辑结构和真实内存对象之间的双向选择联动
...
变量观察器只负责回答“值是什么、怎样变化”。复杂变量可以通过“查看结构”打开以
该变量为根的逻辑结构窗口，也可以通过“查看内存”打开程序级真实内存图。内存图将
栈、全局区和堆分别成尺，地址间的空缺只标记为“未观察”，不会误称为空闲内存；
指针详情固定区分 `&p`（指针自身地址）、`p`（保存的目标地址）和 `*p`（目标值）。
结构无法可靠分类时会降级成通用指针图，不依赖 Agent 猜测名称。

真实内存图始终按照实际地址从低到高排列。数组和二维数组显示连续元素格；结构体
显示字段名、类型、偏移、大小以及编译器插入的对齐填充；链表、二叉树和邻接表仍
显示为分散的节点内存块，并在 `next`、`left/right` 或边指针字段中标出目标对象。
邻接矩阵按连续二维数组展示。逻辑树形或图形布局只在“查看结构”中展示，并与真实
内存块同步选择，避免把逻辑位置误当成物理地址。
...
- Phase 4B：真实指针、堆内存、函数返回值、内存比例尺和通用数据结构视图（已完成基础能力）；继续增加事件驱动算法组件
